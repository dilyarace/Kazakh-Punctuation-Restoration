...
graph LR
    ...
    classDef model fill:#e1f5fe,stroke:#0288d1,stroke-width:1px;
    classDef process fill:#fff3e0,stroke:#f57c00,stroke-width:1px;

    subgraph Data Pipeline
        raw[Raw Kazakh Corpora]:::data --> clean[Text Cleaning]:::process
        clean --> extract[Label Extraction]:::process
        extract --> db[(Processed Dataset)]:::data
    end
    
    subgraph Model Training
        db --> pretrain[Pre-trained Transformer]:::model
        pretrain --> head[Token Classification Head]:::model
        head --> balance[Loss Calculation]:::process
        balance --> checkpoint[Model Checkpoint]:::data
    end
    
    subgraph Inference & Post-processing
        input[Raw Text Input]:::data --> split[Whitespace Split]:::process
        split --> checkpoint
        checkpoint --> predict[Label Prediction]:::process
        predict --> align[Length Alignment]:::process
        align --> output[Punctuated Output]:::data
    end
```
